...
def getListUser():
  ...
  try:
    vDataList = '''
          ...
          '''
    v_hasil = customQuery.selectTanpaParam(vDataList)
    return v_hasil
  except Exception as e:
    return responseJSON(400, 'F', str(e), [])

def proses_hapus_user(user_hapus, tgl_update, no_kta ):
  customQuery = QueryStringDb("POSTGRES")
  try:
    vQueryDelete = '''UPDATE DVD_STORE_USER
          SET USER_AKTIF = 'F', USER_UPDATE = %s, USER_TGL_UPDATE = %s
          WHERE USER_KTA = %s '''
    vKondisi = (user_hapus, tgl_update, no_kta,)
    vHasilDelete = customQuery.execute(vQueryDelete, vKondisi)
    return vHasilDelete
  except Exception as e:
    return responseJSON(400, 'F', str(e), [])
...
def getDetailUser(no_kta):
  ...
  try:
    vDetail = '''
          ...
          '''
    v_kondisi = {"vno_kta":no_kta.upper()}
    v_hasil = customQuery.select(vDetail, v_kondisi)
    return v_hasil
  except Exception as e:
    return responseJSON(400, 'F', str(e), [])

def proses_update(nama, role_f, pin, status_aktif, user_update, tgl_update, no_kta ):
  customQuery = QueryStringDb("POSTGRES")
  try:
    vQuery = '''UPDATE DVD_STORE_USER
          SET USER_NAMA = %s, USER_ROLE = %s, USER_PIN = %s, USER_AKTIF = %s, USER_UPDATE = %s, USER_TGL_UPDATE = %s
          WHERE USER_KTA = %s '''
    vKondisi = (nama, role_f, pin, status_aktif, user_update, tgl_update, no_kta, )
    print(vKondisi)
    vHasilDelete = customQuery.execute(vQuery, vKondisi)
    return vHasilDelete
  except Exception as e:
...
def proses_update_cus(nama, pin, user_update, tgl_update, no_kta):
  customQuery = QueryStringDb("POSTGRES")
  try:
    vQuery = '''UPDATE DVD_STORE_USER
          SET USER_NAMA = %s, USER_PIN = %s, USER_UPDATE = %s, USER_TGL_UPDATE = %s
          WHERE USER_KTA = %s '''
    vKondisi = (nama, pin, user_update, tgl_update, no_kta, )
    print(vKondisi)
    vHasilDelete = customQuery.execute(vQuery, vKondisi)
    return vHasilDelete
  except Exception as e:
